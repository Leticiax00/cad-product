...
<body>
    <!-- Tela cadastro de produto e formatação -->
    <div class="min-h-screen flex items-center justify-center">
        <div class="flex flex-col md:flex-row gap-8 w-full max-w-7xl px-4 py-10">
            <div class="bg-white rounded-2xl shadow-md p-8 flex flex-col justify-center md:h-[350px] overflow-auto w-full max-w-md">
                ...
                </h2> <!-- Formulario do cadastro para envio para o metodo salvar do controler produtos -->
                            ...
                            <div class="flex justify-between items-start">
                                <div class="flex-1">
                                    ...
                                    <div class="flex items-center mt-1 text-gray-600">
                                        <i class="fas fa-tag mr-2"></i> <!-- Ícone Tailwind -->
                                        <span>R$ <?= number_format($produto['preco'], 2, ',', '.') ?></span>
                                        <!-- Print do valor do produto já formatado em preço -->
                                    </div>
                                </div>
                                <div class="flex items-center">
                                    <span class="text-sm px-2 py-1 rounded-full <?= $produto['quantidade'] == 0 ? 'bg-red-100 text-red-800' : ($produto['quantidade'] < 5 ? 'bg-yellow-100 text-yellow-800' : 'bg-green-100 text-green-800') ?>">
                                        <!-- Lógica por trás do estoque menor ou maior que 5 -->
                                        <?= $produto['quantidade'] ?> em estoque <!-- print do estoque -->
                                    </span>
                                </div>
                            </div>
                            <div class="mt-2 flex gap-3">
                                <button class="edit-btn px-3 py-1 text-indigo-600 hover:text-indigo-800" data-id="<?= $produto['id'] ?>" data-nome="<?= esc($produto['nome']) ?>" data-preco="<?= esc($produto['preco']) ?>" data-quantidade="<?= esc($produto['quantidade']) ?>"> <!-- Print e formatação dos produtos -->
                                    <i class="fas fa-edit"></i> Editar
                                </button>
                                <button class="delete-btn px-3 py-1 text-red-600 hover:text-red-800" data-id="<?= $produto['id'] ?>"> <!-- Acessa a id do produto, armazena o valor para imprimi-lo-->
